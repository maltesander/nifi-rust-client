...
```toml
[dependencies]
nifi-rust-client = { version = "0.4", features = ["nifi-2-8-0"] }
```
...
```toml
[dependencies]
nifi-rust-client = { version = "0.4", features = ["dynamic"] }
```
...
**3** NiFi versions tested · **17** added-endpoint checks (17 tested) · **1** enum param checks (1 tested) · **10** field presence checks (1 tested) · **1** query param checks (1 tested)
