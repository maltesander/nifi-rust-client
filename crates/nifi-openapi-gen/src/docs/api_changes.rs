...
const API_CHANGES_START: &str = "<!-- NIFI_API_CHANGES_START -->";
const API_CHANGES_END: &str = "<!-- NIFI_API_CHANGES_END -->";

pub fn emit_api_changes(layout: &RepoLayout, all_specs: &[(String, ApiSpec)]) -> Vec<FileEdit> {
    ...
    vec![FileEdit::CreateOrReplaceBlock {
        path: layout.api_changes_md.clone(),
        start_marker: API_CHANGES_START.into(),
        end_marker: API_CHANGES_END.into(),
        content,
        template,
    }]
}
...
mod tests {
    ...
    #[test]
    fn emit_api_changes_returns_create_or_replace_block() {
        use crate::layout::RepoLayout;
        use crate::plan::FileEdit;
        use std::path::Path;
