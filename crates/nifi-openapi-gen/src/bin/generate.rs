...
fn generate_lib_rs_content(versions: &[&str]) -> String {
    ...
    out.push_str(&format!(
        "compile_error!(\"Enable exactly one NiFi version feature, e.g. features = [\\\"{}\\\"]\");\n\n",
        latest_feature
    ));

    // At-most-one guards: one per pair
    ...
    // Version modules — pub use is gated to exactly this version being active
    // (no other version may be simultaneously enabled) so that --all-features
    // does not produce duplicate-name errors at the module root.
    for (feature, mod_name) in features.iter().zip(mod_names.iter()) {
        ...
        let use_cfg = if other_not.is_empty() {
            ...
            format!("all(feature = \"{feature}\"{other_not})")
        };
        out.push_str(&format!(
            "#[cfg(feature = \"{feature}\")]\nmod {mod_name};\n"
        ));
...
mod tests {
    ...
    fn test_generate_lib_rs_single_version() {
        ...
        assert!(content.contains("mod v2_8_0;"));
        assert!(content.contains("pub use v2_8_0::{api, types};"));
        // No conflict guard needed for a single version
        assert!(!content.contains("#[cfg(all("));
    }
    ...
    fn test_generate_lib_rs_two_versions() {
        ...
        // conflict guard
        assert!(
            content.contains("#[cfg(all(feature = \"nifi-2-7-2\", feature = \"nifi-2-8-0\"))]")
        );
        // both version modules present
        assert!(content.contains("mod v2_7_2;"));
        assert!(content.contains("mod v2_8_0;"));
