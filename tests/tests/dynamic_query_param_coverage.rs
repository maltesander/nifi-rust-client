...
async fn param_flow_get_flow_metrics_flow_metrics_reporting_strategy_accepted() {
    ...
    let result = client.flow_api().get_flow_metrics("prometheus",
        ...
        None,
        None,
        Some(FlowMetricsReportingStrategy::AllProcessGroups)).await;
    assert!(result.is_ok(), "expected param flowMetricsReportingStrategy to be accepted, got: {:?}", result.unwrap_err());
}
