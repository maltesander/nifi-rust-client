...
async fn dynamic_detects_version() {
    ...
    assert!(version.starts_with("2."), "expected NiFi 2.x, got {version}");
}
...
async fn dynamic_about_returns_info() {
    let client = dynamic_logged_in_client().await;
    let about = client.flow_api().get_about_info().await.expect("failed to get about");
...
async fn dynamic_resources_returns_ok() {
    let client = dynamic_logged_in_client().await;
    client.resources_api().get_resources().await.expect("failed to get resources");
}
...
async fn dynamic_current_user_returns_identity() {
    let client = dynamic_logged_in_client().await;
    let user = client.flow_api().get_current_user().await.expect("failed to get current user");
